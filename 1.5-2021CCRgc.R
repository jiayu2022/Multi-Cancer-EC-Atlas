...
setwd("/home/zhangjiayu/project/EC/rawdata/2021CCRgc")
...
sampleid = select(meta, sample)

table(sampleid$sample)

sampleid$cancer = "GC"

sampleid = sampleid %>% mutate(sampleid = str_c(cancer, sample, sep = "_"))

sampleid = select(sampleid, sampleid)

scRNA <- AddMetaData(scRNA, sampleid)
...
scRNA <- CreateSeuratObject(counts = count2, project = "2021_CCR_GC", meta.data = meta,
...
saveRDS(scRNA, file = "/home/zhangjiayu/project/EC/input/2021gcqc1.rds")
